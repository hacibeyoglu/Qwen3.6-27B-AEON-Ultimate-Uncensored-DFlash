...
def patch_gdn_rearrange_capture_safety(pkg_root: Path) -> None:
    ...
    pattern = re.compile(
        r"(?m)^    @torch\.compile\([^\n]*fullgraph=True[^\n]*\)\n"
        r"(?=    def rearrange_mixed_qkv\(self, mixed_qkv\):\n)"
    )
    new = (
        f"    {marker}\n"
    ...
    if count == 0:
        if "def rearrange_mixed_qkv(self, mixed_qkv):" not in text:
            print("[aeon] gdn rearrange_mixed_qkv missing; compile patch skipped")
            return
        print(
            "[aeon] gdn rearrange_mixed_qkv compile decorator not found; "
            "assuming upstream changed or removed the CUDA-graph hazard"
        )
        return
